...
workflow PreProcessingForVariantDiscovery_GATK4 {
  input {
    ...
    Float? alt_mem
    Float SamToFastqAndBwaMem_mem = select_first([alt_mem, 14])
    ...
    String gotc_docker = "arashi-gotc-247"
    String gotc_path = "/usr/gitc/"
    String python_docker = "arashi-py-27"
  ...
  scatter (unmapped_bam in flowcell_unmapped_bams) {
    ...
    call SamToFastqAndBwaMem {
      input:
        mem_size_gb = SamToFastqAndBwaMem_mem,
...
task SamToFastqAndBwaMem {
  ...
  Int command_mems_gb = 1
  Int command_memx_gb = ceil(mem_size_gb) - 2
  ...
  command {
    ...
    java -Dsamjdk.compression_level=~{compression_level} -Xms~{command_mems_gb}G -Xmx~{command_memx_gb}G -jar ~{gotc_path}picard.jar \
    SamToFastq \
    TMP_DIR=tmpdir \
    INPUT=~{input_bam} \
    FASTQ=/dev/stdout \
...
task MergeBamAlignment {
  ...
  Int command_mems_gb = ceil(mem_size_gb/2)
  Int command_memx_gb = ceil(mem_size_gb) - 1
  ...
  command {
    ...
    ~{gatk_path} --java-options "-Dsamjdk.compression_level=~{compression_level} -Xms~{command_mems_gb}G -Xmx~{command_memx_gb}G" \
      MergeBamAlignment \
      --TMP_DIR tmpdir \
...
task SortAndFixTags {
  input {
    ...
    Int command_mems_gb_sort = ceil(mem_size_gb/2)
    Int command_memx_gb_sort = ceil(mem_size_gb) - 1
  ...
  command {
    ...
    ~{gatk_path} --java-options "-Dsamjdk.compression_level=~{compression_level} -Xms~{command_mems_gb_sort}G -Xmx~{command_memx_gb_sort}G" \
      SortSam \
      --TMP_DIR tmpdir \
      --INPUT ~{input_bam} \
      --OUTPUT /dev/stdout \
    ...
    | \
    ~{gatk_path} --java-options "-Dsamjdk.compression_level=~{compression_level} -Xms~{command_mem_gb_fix}G -Xmx~{command_memx_gb_sort}G" \
      SetNmMdAndUqTags \
      --TMP_DIR tmpdir \
      --INPUT /dev/stdin \
      --OUTPUT ~{output_bam_basename}.bam \
...
task BaseRecalibrator {
  ...
  command { 
    ...
    ~{gatk_path} --java-options "-Xms~{command_mems_gb}G -Xmx~{command_memx_gb}G" \
      BaseRecalibrator \
      --tmp-dir tmpdir \
      -R ~{ref_fasta} \
      -I ~{input_bam} \
...
task GatherBqsrReports {
 input { 
   ...
   Int preemptible_tries
   Int disk_size
   Float mem_size_mb = 1024

   String docker_image
   String gatk_path
  }
  Int command_mems_gb = ceil(mem_size_mb/2)
  Int command_memx_gb = ceil(mem_size_mb - 100)

  command {
    mkdir tmpdir

    ~{gatk_path} --java-options "-Xms~{command_mems_gb}m -Xmx~{command_memx_gb}m" \
      GatherBQSRReports \
      --tmp-dir tmpdir \
