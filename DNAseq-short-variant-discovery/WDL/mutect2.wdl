...
workflow Mutect2 {
    input {
      ...
      String funcotator_docker = "arashi-gatk-426"
      File? gatk_override
    ...
    Runtime standard_runtime = {"gatk_docker": gatk_docker, "gatk_override": gatk_override,
            ...
            "machine_mem": small_task_mem * 1000, "command_mem": small_task_mem * 1000,
    ...
    call MergeStats { input: stats = M2.stats, output_name = output_basename, runtime_params = standard_runtime }
    ...
    call Filter {
        input:
            ...
            runtime_params = standard_runtime,
            command_mem = select_first([filter_mem, small_task_mem * 1000]),
    ...
    if (run_funcotator_or_default && send_email && defined(email)) {
        call M2Complete {
            input:
                funcotated_file = Funcotate.funcotated_output_file,
                email = email
        }
    }
...
task MergeBamOuts {
    ...
    command <<<
        ...
        gatk --java-options "-Xmx~{runtime_params.command_mem}m" BuildBamIndex --TMP_DIR tmpdir -I ~{output_vcf_name}.out.bam -VALIDATION_STRINGENCY LENIENT
...
task M2Complete {
    input {
        String? funcotated_file
        String? email
    }
    command {
        echo "~{funcotated_file} generated" | mail -s "Mutect2 run complete" ~{email}
    }
    runtime {
    }
    output {
