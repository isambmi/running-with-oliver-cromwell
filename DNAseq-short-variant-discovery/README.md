...
4. A CSV file ([example](example_inputs/)) containing sample metadata. 
...
- Debug problems with `-d` flag to get ID list of workflows
    ...
    Hint: the `logs` command usually outputs directory of the stdout and stderr logs for a particular workflow. If the output of `oliver inspect` is a cryptic one-liner, it is most likely an issue with the actual input file (wrong path/dir/etc). Try `ls` the path to the file in the `input.json` for a quick test.
...
1. Prepare json input using the provided notebook (section X)
...
1. Generate input.json using notebook
    ...
    - Note: Mutation calling can be made stricter or more permissive by modifying the "arguments" parameter. 
